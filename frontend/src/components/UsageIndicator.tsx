...
export function UsageIndicator({ type, compact = false, showText = true }: UsageIndicatorProps) {
  const { user, getPlanLimits, formatUsageText, getUsagePercentage } = useUserPermissions();
  ...
  const used = isMonthly ? usageStats.imagesGeneratedThisMonth : usageStats.imagesGeneratedToday;
        ...
        <div className="w-full bg-muted rounded-full h-2">
          <div 
            className={`h-2 rounded-full transition-all duration-300 ${
              ...
              percentage >= 70 ? 'bg-amber-500' : 'bg-blue-500'
            }`}

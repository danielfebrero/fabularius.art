...
const IMAGE_SIZES = [
  ...
  { value: '1536x1024', label: '1536×1024 (Landscape)', width: 1536, height: 1024 },
  { value: '1024x1536', label: '1024×1536 (Portrait)', width: 1024, height: 1536 },
...
const LORA_MODELS = [
  { id: 'realistic-enhance', name: 'Realistic Enhancement', description: 'Enhances photo-realism' },
  { id: 'artistic-style', name: 'Artistic Style', description: 'Adds artistic flair' },
  { id: 'detail-boost', name: 'Detail Boost', description: 'Increases fine details' },
  { id: 'color-enhance', name: 'Color Enhancement', description: 'Vibrant colors' },
  { id: 'lighting-master', name: 'Lighting Master', description: 'Perfect lighting' },
];
...
export function GenerateClient() {
  ...
  const handleGenerate = async () => {
    ...
    const promises = Array(settings.batchCount).fill(null).map((_, i) => 
      new Promise<string>(resolve => {
        setTimeout(() => {
          resolve(`https://picsum.photos/${settings.customWidth}/${settings.customHeight}?random=${Date.now()}_${i}`);
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <div className="space-y-3">
          <Textarea
            ...
            onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => updateSettings('prompt', e.target.value)}
        ...
        >
          {isGenerating ? (
            ...
            <div className="flex items-center gap-2">
              ...
              Generate {settings.batchCount > 1 ? `${settings.batchCount} Images` : 'Image'}
            </div>
          )}
        </Button>
        ...
        <div className="space-y-6 pt-4 border-t border-gray-200">
          ...
          {/* Image Size */}
          <div className={`space-y-3 ${!canUseCustomSizes ? 'opacity-50' : ''}`}>
            <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => 
                      updateSettings('customWidth', parseInt(e.target.value) || 1024)
                    }
                    className="h-10"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => 
                      updateSettings('customHeight', parseInt(e.target.value) || 1024)
                    }
                    className="h-10"
                  />
                </div>
          ...
          <div className={`space-y-3 ${!canUseLoras ? 'opacity-50' : ''}`}>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{lora.description}</div>
                    </div>
                    {settings.selectedLoras.includes(lora.id) && canUseLoras && (
            ...
            {settings.selectedLoras.length > 0 && canUseLoras && (
              <p className="text-xs text-blue-600">
                {settings.selectedLoras.length} LoRA{settings.selectedLoras.length !== 1 ? 's' : ''} selected
              </p>
            )}
          </div>

...
# Dump DynamoDB table to local filesystem
# This script will export all items from the DynamoDB table
# and store them in a local directory structure that can be committed to the repo
#
# Usage: ./scripts/dump-dynamodb-table.sh [--env=ENVIRONMENT] [--output=DIRECTORY]
# Examples:
#   ./scripts/dump-dynamodb-table.sh              # Dumps local environment (default)
#   ./scripts/dump-dynamodb-table.sh --env local  # Dumps local environment
#   ./scripts/dump-dynamodb-table.sh --env dev    # Dumps dev environment
#   ./scripts/dump-dynamodb-table.sh --env staging # Dumps staging environment
#   ./scripts/dump-dynamodb-table.sh --env prod   # Dumps prod environment
#   ./scripts/dump-dynamodb-table.sh --env prod --output ./prod-backup # Custom output directory
...
show_usage() {
    ...
    echo ""
    echo "Dump DynamoDB table contents to local filesystem for backup/restore purposes."
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Examples:"
    echo "  $0                                    # Dump local environment table"
    echo "  $0 --env local                       # Dump local environment table"
    echo "  $0 --env prod                        # Dump prod environment table"
    ...
    echo "  ./scripts/restore-dynamodb-table.sh --env local --source ./prod-dump  # Restore to local"
    echo ""
}
...
print_status "Dumping DynamoDB table for environment: $ENVIRONMENT"
...
if ! aws_cmd dynamodb describe-table --table-name "$TABLE_NAME" >/dev/null 2>&1; then
    print_warning "Table '$TABLE_NAME' does not exist. Nothing to dump."
    exit 0
fi
...
print_status "Getting table item count..."
ITEM_COUNT=$(aws_cmd dynamodb describe-table --table-name "$TABLE_NAME" --query 'Table.ItemCount' --output text 2>/dev/null || echo "0")

if [ "$ITEM_COUNT" = "0" ] || [ "$ITEM_COUNT" = "None" ]; then
    print_warning "Table '$TABLE_NAME' is empty. Nothing to dump."
    exit 0
fi

print_status "Found $ITEM_COUNT items to dump"
...
# Export table data using scan operation
print_status "Exporting table data..."
DATA_FILE="$DUMP_DIR/table-data.json"

# Use scan to get all items
if aws_cmd dynamodb scan --table-name "$TABLE_NAME" --output json > "$DATA_FILE"; then
    print_success "Table data exported successfully"
else
    print_error "Failed to export table data"
    exit 1
fi

# Get table description
print_status "Exporting table schema..."
SCHEMA_FILE="$DUMP_DIR/table-schema.json"
if aws_cmd dynamodb describe-table --table-name "$TABLE_NAME" --output json > "$SCHEMA_FILE"; then
    print_success "Table schema exported successfully"
else
    print_warning "Failed to export table schema, continuing anyway..."
fi
...
- table-schema.json: Table schema and configuration
- table-metadata.json: Table metadata and dump information
- dump-summary.txt: This summary file

To restore this dump, run: ./scripts/restore-dynamodb-table.sh --env=$ENVIRONMENT
EOF
...
echo ""
print_success "🎉 DynamoDB table dump completed successfully!"
echo ""
...
echo ""
echo "📝 Files created:"
echo "  • $DUMP_DIR/table-data.json - All table items"
echo "  • $METADATA_FILE - Table metadata"
echo "  • $SCHEMA_FILE - Table schema"
echo "  • $SUMMARY_FILE - Dump summary"
echo ""
echo "💡 Tip: You can now commit the '$DUMP_DIR' directory to your repo"
echo "💡 To restore: ./scripts/restore-dynamodb-table.sh --env=$ENVIRONMENT"

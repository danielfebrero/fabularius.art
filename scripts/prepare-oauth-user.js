...
async function prepareOAuthUser(environment, email) {
  try {
    ...
    console.log(
      `Preparing OAuth user: ${email} in ${environment} environment`
    );
...
if (args.length !== 2) {
  ...
  console.error("Use update-user-role.js to assign specific roles after creation.");
  process.exit(1);
}
...
if (!environment || !email) {
  console.error(
    "All parameters (environment, email) are required"
  );
  process.exit(1);
}
...
try {
  ...
  if (environment === "prod") {
    ...
    console.log(
      "⚠️  Make sure this is intentional and the email is correct."
    );
    console.log("");
  }
